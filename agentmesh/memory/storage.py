...
class MemoryStorage:
    ...
    def _init_db(self):
        ...
        # Create indexes
        self.conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_chunks_agent_user 
            ON chunks(agent_id, user_id)
        """)
        ...
        self.conn.execute("""
            CREATE TRIGGER IF NOT EXISTS chunks_ai AFTER INSERT ON chunks BEGIN
                INSERT INTO chunks_fts(rowid, text, id, agent_id, user_id, path, source, scope)
                VALUES (new.rowid, new.text, new.id, new.agent_id, new.user_id, new.path, new.source, new.scope);
            END
        """)
        ...
        self.conn.execute("""
            CREATE TRIGGER IF NOT EXISTS chunks_au AFTER UPDATE ON chunks BEGIN
                UPDATE chunks_fts SET text = new.text, id = new.id, agent_id = new.agent_id,
    ...
    def save_chunk(self, chunk: MemoryChunk):
        ...
        self.conn.execute("""
            ...
            (id, agent_id, user_id, scope, source, path, start_line, end_line, text, embedding, hash, metadata, updated_at)
    ...
    def save_chunks_batch(self, chunks: List[MemoryChunk]):
        ...
        """, [
            (
                c.id, c.agent_id, c.user_id, c.scope, c.source, c.path,
    ...
    def delete_by_path(self, agent_id: str, path: str):
        """Delete all chunks from a file"""
        self.conn.execute("""
            DELETE FROM chunks WHERE agent_id = ? AND path = ?
        """, (agent_id, path))
        self.conn.commit()
    
    def get_file_hash(self, agent_id: str, path: str) -> Optional[str]:
        """Get stored file hash"""
        row = self.conn.execute("""
            SELECT hash FROM files WHERE agent_id = ? AND path = ?
        """, (agent_id, path)).fetchone()
        return row['hash'] if row else None
    
    def update_file_metadata(self, agent_id: str, path: str, source: str, file_hash: str, mtime: int, size: int):
        """Update file metadata"""
        self.conn.execute("""
            INSERT OR REPLACE INTO files (path, agent_id, source, hash, mtime, size, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, strftime('%s', 'now'))
        """, (path, agent_id, source, file_hash, mtime, size))
        self.conn.commit()
    
    def get_stats(self, agent_id: str) -> Dict[str, int]:
        """Get storage statistics"""
        chunks_count = self.conn.execute("""
            SELECT COUNT(*) as cnt FROM chunks WHERE agent_id = ?
        """, (agent_id,)).fetchone()['cnt']
        
        files_count = self.conn.execute("""
            SELECT COUNT(*) as cnt FROM files WHERE agent_id = ?
        """, (agent_id,)).fetchone()['cnt']
